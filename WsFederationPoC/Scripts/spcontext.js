(function (window, undefined) {
    ...
    var SPLanguageKey = "SPLanguage";
    var SPClientTag = "SPClientTag";
    var SPProductNumber = "SPProductNumber";
    ...
    $(document).ready(function () {
        ...
        var spHostUrl = getSPHostUrlFromQueryString(window.location.search, SPHostUrlKey);
        var spLanguage = getSPHostUrlFromQueryString(window.location.search, SPLanguageKey);
        var spClientTag = getSPHostUrlFromQueryString(window.location.search, SPClientTag);
        var spProductNumber = getSPHostUrlFromQueryString(window.location.search, SPProductNumber);

        var currentAuthority = getAuthorityFromUrl(window.location.href).toUpperCase();

        if (spHostUrl && currentAuthority) {
            appendSPHostUrlToLinks(spHostUrl, currentAuthority, SPHostUrlKey);
            appendSPHostUrlToLinks(spLanguage, currentAuthority, SPLanguageKey);
            appendSPHostUrlToLinks(spClientTag, currentAuthority, SPClientTag);
            appendSPHostUrlToLinks(spProductNumber, currentAuthority, SPProductNumber);
        }
    });

    // Appends SPHostUrl as query string to all the links which point to current domain.
    function appendSPHostUrlToLinks(spHostUrl, currentAuthority, key) {
        $("a")
            .filter(function () {
            ...
            .each(function () {
                if (!getSPHostUrlFromQueryString(this.search, key)) {
                    if (this.search.length > 0) {
                        this.search += "&" + key + "=" + spHostUrl;
                    }
                    else {
                        this.search = "?" + key + "=" + spHostUrl;
                    }
    ...
    function getSPHostUrlFromQueryString(queryString, key) {
        if (queryString) {
            if (queryString[0] === "?") {
